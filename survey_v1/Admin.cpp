...
void Admin::adminPortal(){
    ...
    do {
        cout<<"\n\nMenu\n"
            <<"Press 1: Print all Users\n"
            <<"Press 2: Find one index\n"
            <<"Press 3: Find by email\n"
            <<"Press 4: Edit binary\n"
            <<"Press 5: Delete 1 record\n"
            <<"Press 6: Update Admin\n"
            <<"Press 9: Logout\nEnter a number: ";
        cin>>choice;
        //cin.ignore();
...
void Admin::getAllUsrs(){
    ...
    // create pointer to array of Admin instances
    //Admin *ad = new Admin[nRec]; 
    //Admin ad;
    ...
    while(count < nRec){         
        ...
        inBin.seekg(cursor,ios::beg);   // sets cursor at the beginning of the file we're looking for
        
        // Read & set ttlRec 
        inBin.read(reinterpret_cast<char *>(&ttlRec) , sizeof(int)); 
        thisSum += sizeof(int);         
        cout<<"\ncount = " << count << "  ttlRec = "<< ttlRec;
        ...
        if(count==0){
            ...
            setCursLoc(cursor-thisSum);
        }
        
        //printUser();
        //cout<<"\nrecSiz = "<<recSiz<<" begnFile = "<<begnFile<<"   count = "<<count<<endl;   
        count++; 
        thisSum = 0L;
   }
...
void Admin::deleteUsr(){
    ...
    //cout<<"\nTotal records = "<< getTtlRec() <<" Record #"<< ad.ttlRec<<endl;
    
//    while( count < getTtlRec()){
//        
//        if(count == ad.ttlRec){
//            cout<<"Skip this record\n";
//        
//        } else {
//            //wrtTxt("temp.txt");
//           //wrtBin();
...
void Admin::reWrtTxt(){
    ...
    int charCount = (ttlRec==0) ? 0 : ((86)*ttlRec);
    //cout<<"\nRec # = "<<ttlRec<<endl;
...
void Admin::read1Bin(){
    ...
    cout<<"Hit read1Bin()\n";
    cout<<"recSiz = "<<recSiz<<"  begnFile = "<<begnFile<<endl;
